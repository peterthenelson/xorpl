...
/// written to disk and compiled independently of this crate.  The file
/// includes a "do not edit" header and the emitted function.
pub fn emit_rust(vm: &ConcreteVm, fn_name: &str, param_names: &[&str]) -> String {
    ...
    // Function signature
    let sig_params = param_names.iter()
        .map(|p| format!("{p}: u32"))
        .collect::<Vec<_>>()
        .join(", ");
